...
-- e. Create a function to get the quarter

CREATE DEFINER=`root`@`localhost` FUNCTION `get_quarter`(quarter_date date) RETURNS char(2) CHARSET latin1
    DETERMINISTIC
BEGIN
  Declare fiscal_qtr tinyint;
  Declare qtr char(2);
  set fiscal_qtr = month(quarter_date);
  Case
  
  when fiscal_qtr in(9,10,11) then  set qtr = "Q1";
  when fiscal_qtr in(12,1,2) then set qtr = "Q2";
  when fiscal_qtr in(3,4,5) then set qtr = "Q3";
  else set qtr = "Q4";
  end  case;
